...
Runs cuBLAS, Triton, WMMA, Raw SIMT, and CuTile.
Auto-detects GPU (tested on RTX PRO 6000 Blackwell Server Edition).
Saves comprehensive results with hardware config, software versions,
and implementation descriptions to results/GEMM/.
"""
...
RESULTS_DIR = Path(__file__).parent.parent / "results_RTX_PRO_6000" / "GEMM"
...
IMPL_DESCRIPTIONS = {
    ...
    "WMMA": {
        ...
        "description": (
            ...
            "double-buffered cp.async pipeline (NUM_STAGES=2 for sm_120 shared memory limit), "
        ...
        "sm_120_fix": "NUM_STAGES reduced from 3 to 2 (48KB shared mem limit on sm_120)",
    },
    "Raw SIMT": {
...
def run_benchmarks():
    ...
    from gemm_cutile import HAS_CUTILE

    cutile_fn = None
    if HAS_CUTILE:
        from gemm_cutile import gemm_cutile as cutile_fn
    ...
    impl_order = ["cuBLAS", "Triton", "WMMA", "Raw SIMT", "CuTile"]
    print("\n" + "=" * 90)
