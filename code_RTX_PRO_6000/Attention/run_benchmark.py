...
RESULTS_DIR = Path(__file__).parent.parent / "results_RTX_PRO_6000" / "Attention"
...
def run_benchmarks():
    ...
    HAS_CUTILE_ATTN = False
    cutile_fn = None
    try:
        from attn_cutile import attn_cutile, HAS_CUTILE
        if HAS_CUTILE:
            cutile_fn = attn_cutile
            HAS_CUTILE_ATTN = True
            print("  CuTile attention: loaded", flush=True)
    except Exception as e:
        print(f"  CuTile attention: unavailable ({e})", flush=True)

    implementations = [
        ("FlashAttention-2",  attn_flash_sdpa,  True),
        ("cuDNN (SDPA)",      attn_cudnn_sdpa,   True),
        ("Triton",            triton_fn,         HAS_TRITON_ATTN),
        ("CuTile",            cutile_fn,         HAS_CUTILE_ATTN),
    ...
    # Paper table
    impl_order = ["FlashAttention-2", "cuDNN (SDPA)", "Triton", "CuTile", "Naive (Unfused)"]
    print("\n" + "=" * 90)
